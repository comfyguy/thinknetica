class Station
  ...
  def trains_by_type(type)
    raise 'Type should be a non empty string' unless type.is_a?(String) && type.size > 0
    output = []
  ...
  def validate!
    raise 'Station name should be a non empty string' unless id.is_a?(String) && id.size > 0
    true
  end
end
